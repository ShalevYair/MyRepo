...
2. **STEPLIB** — יש קונפיגורציה (NATPARM/JCL) עם שרשרת ה-STEPLIB?
   בלעדיה חלק מהקשתות יישארו `ambiguous`. **עדכון (2026-09-03, `jclmap.py` על 237 קבצי JCL אמיתיים):**
   רק **קובץ אחד** מתוך 237 מכיל `STEPLIB`/`NATLIB` DD בכלל. זו לא תשובה סופית (יכולה
   להיות קונפיגורציה ב-NATPARM שלא ב-JCL כלל) — אבל בהנחה שאין מקור אחר, כמעט כל
   הקשתות ב-`natmap3.py` (שלב 3) יישארו `ambiguous`. אם יש NATPARM עם STEPLIB — זה
   המקור שצריך.
3. ~~**"PCL"**~~ **נענה (2026-09-03):** אין PL/I ב-estate. PCL היא תיקיית COBOL
   קטנה שהמשתמש קיבל בנפרדת מ-CICS (במאגר גולמי בן ~1000MB, בלי כל תיעוד) ומיזג
   ידנית לתוך תיקיית CICS — הוא עצמו לא יודע למה היא הייתה נפרדת במקור.
   **לא משנה לניתוח**: הזיהוי של `cobolmap.py` (§1.1/§4.2, מפורט ב-`parseCobol`
   הקיים ב-`app.js`) הוא לפי **תוכן** כל קובץ (`01 SAP-OPTIONS TPMONITOR
   UTP-CICS.`), לא לפי מיקומו בתיקייה — המיזוג הידני לא פוגע בזיהוי CICS.
   שווה לשמור בצד את הרשימה המקורית של אילו קבצים היו תחת PCL (למשל בקובץ טקסט),
   ליתר ביטחון, למקרה שיתגלה בהמשך שכן היה להפרדה משמעות (סימן: כל קבצי PCL
   שייכים לאותו תת-מוצר/דומיין עסקי אחד שאינו קשור ל-CICS).
...
7. **הרצת Python** — על איזו מכונה? יש גישה ל-800MB שם? יש הגבלת רשת
   ליציאה ל-Gemini?
8. **DDM + DDM OUTPUT** (עלה 2026-09-03) — במאגר הגולמי שהתקבל יש גם קבצי DDM
   וגם קבצי "DDM OUTPUT" נפרדים, שהמשתמש רוצה שייכנסו לניתוח. **טרם נבדק מול
   דגימה אמיתית** — השערה בלבד, לפי מה שכבר מתועד:
   * **DDM** — סביר שזהו unload גולמי באותו פורמט בדיוק (`*H**`/`*C**`/
     `*D01-04`/`*S**`) עם `type=V`, בדיוק כמו שמתואר ב-README.md
     ("DDM has the same raw+report pairing as the programs above") — אם כך,
     `natunload_split.py` (שלב 1.1, כבר קיים) כבר תומך בזה ללא שום שינוי;
     צריך רק להריץ אותו גם על קובץ ה-DDM.
   * **DDM OUTPUT** — סביר שזהו דוח job-log מקביל (132 עמודות קבועות),
     שה-README מתעד ל-DDM ("Software AG's own DDM job-log report uses the
     identical 132-column table, just with TYPE=DDM") — `natlib/report_profile.py`
     (שלב 0.3, כבר קיים) כבר מזהה את הפורמט הזה, אבל **אין עדיין תוכנית
     שצורכת אותו** (`ingest_report.py`, שלב 4.3 ב-WORKPLAN.md, לא נכתב).
   **לפני שממשים משהו**: לבדוק דגימה אמיתית משני הקבצים (כמו שנעשה לשלב 1.1) —
   אם ההשערה נכונה, אין קוד חדש לכתוב מעבר להרצה נוספת; אם לא, זה פורמט שלישי
   שדורש עבודה נפרדת.
